...
from utils import to_psnr_test, print_log, validation, adjust_learning_rate, to_psnr
...
def main(test_img, test_gt, test_phrase, test_epoch):
    ...
    def save_image(dehaze, image_name, category):
        ...
        for ind in range(batch_num):
            utils.save_image(dehaze[ind], '{}_results/{}'.format(category, image_name[ind][:-3] + 'png'))
    ...
    if test_phrase == 1:
        ...
        G1.load_state_dict(torch.load('./checkpoint/1_'+str(test_epoch)+'.tar'))
        G1.eval()
        psnr=[]
        net_time = 0.
        net_count = 0.
